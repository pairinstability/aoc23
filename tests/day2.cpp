...
{
    Day2 day2;
    auto sample_sol_a = day2.SolvePart1(PATH "input/day2a_sample.txt");

    EXPECT_TRUE(sample_sol_a.has_value());
    EXPECT_EQ(sample_sol_a.value(), 8.0);

    auto puzzle_sol_a = day2.SolvePart1(PATH "input/day2_puzzle.txt");
    EXPECT_TRUE(puzzle_sol_a.has_value());
    GTEST_COUT << "day 2a: " << puzzle_sol_a.value() << std::endl;

    auto sample_sol_b = day2.SolvePart2(PATH "input/day2a_sample.txt");

    EXPECT_TRUE(sample_sol_b.has_value());
    EXPECT_EQ(sample_sol_b.value(), 2286.0);

    auto puzzle_sol_b = day2.SolvePart2(PATH "input/day2_puzzle.txt");
